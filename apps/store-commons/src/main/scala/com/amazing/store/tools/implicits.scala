package com.amazing.store.tools.implicits
...
import com.amazing.store.tools.{IdGenerator, Reference}
...
package object flatfutures {
  ...
  implicit final class futureOfOptionToFuture[A](future: Future[Option[A]]) {
    ...
    def flatten(none: Throwable)(implicit ec: ExecutionContext): Future[A] = {
      future.flatMap {
        case Some(something) => Future.successful(something)
        case None => Future.failed(none)
      }
    }
    ...
    def flatten[O](some: A => O)(none: Throwable)(implicit ec: ExecutionContext): Future[O] = {
      future.flatMap {
        case Some(something) => Future.successful(some(something))
        case None => Future.failed(none)
      }
